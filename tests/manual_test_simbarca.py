...
def plot_label_scatter(data_sequence:torch.Tensor, save_dir="./", section_num=100, save_note="segment"):
    
    p = section_num
    S, T, P = data_sequence.shape # num_sim_sessions, num_time_steps, total_num_sections
    
    fig, ax = plt.subplots(figsize=(6, 4))
    pred_speed_sec = data_sequence[:, :, p]
    xx = np.arange(T) * 3
    ...
    fig.tight_layout()
    fig.savefig("{}/label_clusters_{}_{}.pdf".format(save_dir, p, save_note))
    plt.close()
...
class TestSimBarca(unittest.TestCase):
    ...
    def test_full_data_loading(self):
        ...
        for num in [113, 345, 1068]:
            # visualize_batch(batch, section_num=section_id_to_index[section_id])
            plot_label_scatter(test_set.pred_speed, section_num=num)
